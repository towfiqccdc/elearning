...
The panel on the right side of the homepage hero auto-advances through
4 images every 2 seconds. Right now it shows original placeholder
illustrations (`images/hse-demo-1.svg` through `hse-demo-4.svg`) — swap
these for real photos whenever you have them:

1. Add your photo files anywhere in the `images/` folder
2. In `index.html`, find the four `<img class="carousel-slide" ...>` tags
   near the top and change each `src` to your new filename

No other file needs to change — the carousel logic works with any number
of images as long as the `src` paths are valid.

## Color theme

The site uses CNPC's brand colors (red `#DA251D` and orange `#E05724`,
based on the company's actual logo) as accents, on a white-dominant
background — matching Chuanqing Drilling's corporate identity. All colors
are defined once at the top of `css/style.css` under `:root`, so the
whole palette can be adjusted from that one place if needed.
